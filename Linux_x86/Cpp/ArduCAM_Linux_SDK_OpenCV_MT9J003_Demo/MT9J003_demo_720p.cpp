...
//Compile with command: g++ MT9J003_demo_720p.cpp -o Test `pkg-config --cflags --libs opencv` -lArduCamLib -lusb-1.0  -lpthread -L. -I. -std=gnu++11
...
void *fnDisplay_Thread(void *para)
{
	...
	if ( ArduCam_available(useHandle) > 0)
	{
	        ...
	        //6. Display the frame
	        raw_image = BytestoMat(pu8FrameData, 1408, 792);
	        if ( !raw_image.data )
		{
			cout << "No image data \n";
			//return -1;
...
int main(int argc, char** argv )
{
    ...
    useCfg.u32Width = 1408;
    useCfg.u32Height = 792;
    ...
    ArduCam_writeSensorReg( useHandle,0x0304, 0x07);   		// pre_pll_clk_div = 0x4
    ArduCam_writeSensorReg( useHandle,0x0306, 0x50);   		// pll_multiplier = 0x6E
    ArduCam_writeSensorReg( useHandle,0x0308, 0x08);   		// op_pix_clk_div = 0xC
    ...
    ArduCam_writeSensorReg( useHandle,0x0344, 0x218);	     	// Column Start = 0x70
    ArduCam_writeSensorReg( useHandle,0x0348, 0xD15);	     	// Column End = 0xEBF
    ArduCam_writeSensorReg( useHandle,0x0346, 0x24E);	     	// Row Start = 0x8
    ArduCam_writeSensorReg( useHandle,0x034A, 0x87B);	     	// Row End = 0xAC3
    ArduCam_writeSensorReg( useHandle,0x3040, 0x00C3);	     	// Read Mode = 0x41
    ArduCam_writeSensorReg( useHandle,0x0400, 0x0);   		// Scaling Mode = 0x0
    ArduCam_writeSensorReg( useHandle,0x0404, 0x10);   		// Scale_M = 0x10
    ArduCam_writeSensorReg( useHandle,0x034C, 0x580);	    	// Output Width = 0xE50
    ArduCam_writeSensorReg( useHandle,0x034E, 0x318);	     	// Output Height = 0xABC
    ArduCam_writeSensorReg( useHandle,0x0342, 0x0B70);	     	// Line Length = 0x1D10
    ArduCam_writeSensorReg( useHandle,0x0340, 0x07C3);	     	// Frame Lines = 0xB4D
    ...
    ArduCam_writeSensorReg( useHandle,0x3ED0, 0x5BA4);	     	// Summing Procedure 2 = 0x1B24
    ArduCam_writeSensorReg( useHandle,0x3EDC, 0xC3E4);	     	// Summing Procedure 3 = 0xC3E4
    ArduCam_writeSensorReg( useHandle,0x3EE8, 0x1400);	     	// Summing Procedure 4 = 0x0
