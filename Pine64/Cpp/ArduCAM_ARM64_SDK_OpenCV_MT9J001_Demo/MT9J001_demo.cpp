//Arducam SDK multi-thread example for MT9J001 monochrome camera
//This demo is only demonstrate how to use the Arducam SDK functions, 
//Tested under Pine64 ubuntuMATE16.04 OS Opencv 2.4.13
//Compile with command: g++ MT9J001_demo.cpp -o Test `pkg-config --cflags --libs opencv` -lArduCamLib -lusb-1.0  -lpthread -L. -I. -std=gnu++11
...
void *fnDisplay_Thread(void *para)
{
	...
	if ( ArduCam_available(useHandle) > 0)
	{
		...
		resize(raw_image, resize_image, cv::Size(640, 480), (0,0), (0,0), cv::INTER_NEAREST);
...
int main(int argc, char** argv )
{
    ...
    ArduCam_writeSensorReg( useHandle,0x0206, 0x0008);		//analogue_gain_code_greenr
    ArduCam_writeSensorReg( useHandle,0x0208, 0x000A);		//analogue_gain_code_red
    ArduCam_writeSensorReg( useHandle,0x020A, 0x000A);		//analogue_gain_code_blue
    ArduCam_writeSensorReg( useHandle,0x020C, 0x0008);		//analogue_gain_code_greenB
